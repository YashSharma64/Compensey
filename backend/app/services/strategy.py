...
def generate_strategic_response(company_a, company_b, metrics_a, metrics_b, question, drivers: Optional[List[str]] = None):
    metrics_a, metrics_b = metrics_a or {}, metrics_b or {}
    ...
    def delta_phrase(metric: str, delta: float) -> str:
        ...
        return f"{metric} is {label} ({'higher' if delta > 0 else 'lower'} by ~{fmt(abs(delta))})"
    ...
    summary_focus = {"outlook": "outlook", "risk": "risk", "recommendation": "recommended action", "drivers": "key drivers", "general": "positioning"}[intent]

    summary = f"{opener} ({summary_focus}, {timeframe}): {company_a} vs {company_b}. " \
    ...
    insight = f"Key insight: {delta_phrase('growth', growth_delta)}; {delta_phrase('sentiment', sent_delta)}; " \
              f"risk is {'higher' if risk_delta > 0 else 'lower' if risk_delta < 0 else 'similar'} for {company_a} by ~{fmt(abs(risk_delta))}."

    driver_line = "Drivers: " + "; ".join([d.strip() for d in drivers if d.strip()][:2]) if drivers else None
    ...
    if growth_leader == sentiment_leader:
        leader, follower = growth_leader, company_b if growth_leader == company_a else company_a
        recommendation = f"Recommendation: {leader} should press advantage by scaling what is repeatable (quality + cadence), not just acquisition. " \
                         f"{follower} should focus on one wedge: fix experience to close sentiment, or build a distinct growth channel (not imitation)."
    else:
        recommendation = f"Recommendation: {growth_leader} should convert momentum into trust (reduce friction, improve reliability) to avoid a sentiment tax. " \
                         f"{sentiment_leader} should translate goodwill into a repeatable growth engine (distribution/pricing/partnerships)."

    risk_note = f"Risks: sentiment reversal for the growth leader at scale; growth stalling for the sentiment leader; volatility spikes (today {_risk_label(risk_a)} vs {_risk_label(risk_b)})."
    next_step = _pick(rng, ("Next step: validate drivers behind sentiment (top recurring pain points) and confirm growth durability across multiple periods.",
    ...
    question_wrap = f"In your question ('{question}'), the decision hinge is whether the leader can sustain its advantage while closing its gap." if question else ""

    parts = [summary, insight]
    if driver_line: parts.append(driver_line)
    if tradeoff: parts.append(tradeoff)
    parts.extend([recommendation, risk_note, next_step])
    if question_wrap: parts.append(question_wrap)
