...
def _cached_compare(company_a: str, company_b: str):
    start_time = time.time()
    logger.info(f"[compare] Cached process starting for: {company_a} vs {company_b}")
    
    df_a = load_company_data(company_a)
    df_b = load_company_data(company_b)
    ...
    noise_a = (_get_hash(company_a) - 0.5) * 0.2
    noise_b = (_get_hash(company_b) - 0.5) * 0.2
    ...
    explanations = explain_decision(
        company_a, company_b, metrics_a, metrics_b,
...
async def compare_companies(request: CompareRequest):
    try:
        # Utilizing cache to prevent repeated long computations / LLM requests
        return _cached_compare(request.company_a, request.company_b)
